...
public class PreviousMessageValidator implements IMessageValidator {
    private static final Logger LOGGER = LoggerFactory.getLogger(PreviousMessageValidator.class);
    private final IRocketStates rocketStates;

    @Inject

    public PreviousMessageValidator(IRocketStates rocketStates) {
    ...
    public boolean validate(Message msg) throws MessageValidatorException {
        ...
        if (lastMsgNbr > newMsgNbr) {
            String logMsg = String.format("%s - Wrong message order: Received a previous message. Last message number: %s, New message number %s", msg.getChannel(), lastMsgNbr, newMsgNbr);
            LOGGER.info(logMsg);
